...
class ALL2():
	def __init__(self):
		...
		Three commands to use:
				 ...
				 call --> Based on score cut-off generates sample level files/plots for mosaic,germline mutations and plots variant allele frequency, mutation spectrum				
	...
	def apply_score_argument_parse(self):
		...
		parser.add_argument("-g","--get_score_directory",help="Path to output directory of the get_score option",required="True",type=Util.DirectoryValidator)
		parser.add_argument("-r","--reference",help="Path to reference file",required="True",type=Util.FileValidator)
		parser.add_argument("-o","--output_dir",help="Path to directory where results will be written",required="True")
		parser.add_argument("-a","--af_cutoff",help="Allele frequency cut-off for variants (default=0.35)",default="0.35")
	...
	def explaination_score(self, variant_dict, pairs_vaf_dict, list_of_samples,output_dir):
		...
		output_file_fh.write("#Chrom\tPos\tRef\tAlt\tMosaic_score\tGermline_score\tNumber_of_samples_with_mutation\tSamples_with_mutation\tVAF_of_samples_with_mutation\tNumber_of_comparision_per_sample\n")
	...
	def plot_bar(self, vaf_dict, af_cutoff, output_dir):
		...
		title="Number of mutations above allele frequency cutoff of "+str(af_cutoff)
		...
		output_file=os.path.join(output_dir,"per_sample_mutation_count.png")
	...
	def create_spectra_file(self, vaf_dict, sample, variant_type, af_cutoff, output_dir, reference):
		...
		for i in open(list_of_mut):
			...
			if mut_sig in sample_signature_count:
				spectra_line=mut_sig+"\t"+str(float(float(sample_signature_count[mut_sig])/float(total_variant))*100)+"\t"+str(sample_signature_count[mut_sig])+"\n"
			else:
